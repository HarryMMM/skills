...
### 分层约束
```
tools/apis → services → repositories
```
- tools/apis：禁止直接访问数据库
- services：禁止直接执行 SQL
- repositories：承担所有数据访问，SQL 必须参数化

### 技术栈
- 配置：`pydantic-settings`（域分离）
- 日志：标准 `logging` + 彩色 Formatter
- 数据访问：`SQLAlchemy 2.x` async
- 默认数据库：`aiosqlite`（内存 SQLite，开箱即用）
- MCP：`mcp` SDK (FastMCP)
...
│       │   └── apis/example_router.py.template
│       └── scripts/
│           └── seed_data.py.template
└── scripts/                          # 验证脚本
