...
### 可选（默认值）
- 工程创建位置：默认在当前目录下新建工程子目录（也可选择直接在当前目录创建）
- Python 版本：默认 `3.13`
- 包名/模块名：默认由项目名规范化
...
所有流程均采用**引导式交互**：逐步提问、逐步确认、每阶段反馈进度。

### new：新建工程
1. 开场说明，告知用户需要收集的信息
2. 依次询问：项目名称、项目定位
3. 推断工程形态，展示结果并等待确认
4. 询问工程创建位置（当前目录 / 新建子目录）
5. 告知可选配置（可跳过）
6. 汇总所有信息，等待用户最终确认
7. 创建目录结构、生成配置文件、复制骨架代码（每步反馈进度）
8. 执行验证流程
9. 输出结果与人工复核清单
...
1. 开场说明，告知用户流程步骤
2. 扫描现有结构（反馈进度）
3. 展示差异清单与建议操作，等待用户确认
4. 按“结构 → 基础设施 → 分层代码”顺序改造（反馈进度）
5. 保持行为兼容，避免无关重构
6. 执行验证流程
7. 输出改造摘要、兼容说明与复核清单
...
### feature：新特性开发
1. 开场说明，告知用户流程步骤
2. 依次询问：特性目标、输入输出、边界条件
3. 分析影响范围，展示落点文件方案，等待用户确认
4. 按分层约束实现功能（反馈进度）
5. 补充必要测试或最小验证样例
6. 执行验证流程
7. 输出功能变更摘要、风险点与复核清单
